...
This document describes the split between the two related repositories:
...
flowchart TD
    subgraph Windows[Windows / IIS Control Plane]
        ...
        WIN[Windows Event Log + Deployment Scripts]
    end
    ...
    subgraph Boundary[Integration Boundary]
        ...
        IF3[IPolicyService]
        CFG[DefenseEngine Options + Validators]
    end
...
- **`LinuxDefenseEngineClient`**: HTTP adapter with timeout/retry, cancellation-aware backoff, and fallback behavior.
...
- **`DefenseEngineSyncService`**: periodic pull/push synchronization with configurable interval.
- **`DefenseEngineOptionsValidator`**: startup validation for endpoint/timeout/retry settings.
...
Route mappings are configurable in `DefenseEngine:ApiRoutes` to avoid hardcoding API paths in orchestration code.

...
This repository is moving toward a pure-.NET version of the upstream `ai-scraping-defense` platform. Commercial v1 now uses a single deployable ASP.NET Core host backed by explicit .NET project boundaries for edge, escalation, tarpit, and shared contracts.

## Current Components

### Solution Layout

- `AiScrapingDefense.EdgeGateway`: the ASP.NET Core host assembled from [RedisBlocklistMiddlewareApp/Program.cs](../RedisBlocklistMiddlewareApp/Program.cs)
- `AiScrapingDefense.Contracts`: shared configuration, request/decision models, and cross-project interfaces
- `AiScrapingDefense.EscalationEngine`: threat scoring, reputation providers, and model adapters
- `AiScrapingDefense.TarpitApi`: deterministic tarpit rendering and PostgreSQL-backed Markov content
...
The background worker in [RedisBlocklistMiddlewareApp/Services/DefenseAnalysisService.cs](../RedisBlocklistMiddlewareApp/Services/DefenseAnalysisService.cs) performs the first .NET-native version of the upstream AI-service and escalation-engine flow. Threat scoring itself now lives in [AiScrapingDefense.EscalationEngine/Services/ThreatAssessmentService.cs](../AiScrapingDefense.EscalationEngine/Services/ThreatAssessmentService.cs):
...
Peer signal exchange is now represented by [RedisBlocklistMiddlewareApp/Services/PeerSyncService.cs](../RedisBlocklistMiddlewareApp/Services/PeerSyncService.cs) and the authenticated export endpoint in [RedisBlocklistMiddlewareApp/Program.cs](../RedisBlocklistMiddlewareApp/Program.cs).

Current behavior:

- exports recent blocked defense decisions at `/peer-sync/signals` behind a dedicated peer API key
- imports remote peer signals on a timer from explicitly configured peers
- supports legacy peer IP lists and richer signal envelopes
- applies an explicit trust policy per peer:
  - `ObserveOnly` records imported signals without blocking
  - `BlockList` imports valid peer signals directly into the Redis blocklist
- exposes peer-sync status through the authenticated management API

### Tarpit Surface

Requests rewritten to `/anti-scrape-tarpit/{path}` are served by the minimal API endpoint in [RedisBlocklistMiddlewareApp/Program.cs](../RedisBlocklistMiddlewareApp/Program.cs). Content is generated by [AiScrapingDefense.TarpitApi/Services/TarpitPageService.cs](../AiScrapingDefense.TarpitApi/Services/TarpitPageService.cs).

Current behavior:

- Deterministic HTML based on request path and seed.
- Optional PostgreSQL-backed Markov paragraphs loaded through [AiScrapingDefense.TarpitApi/Services/PostgresTarpitMarkovStore.cs](../AiScrapingDefense.TarpitApi/Services/PostgresTarpitMarkovStore.cs).
...
This solution is intentionally shaped so it can evolve from a single deployable into separate .NET services as parity work continues:
